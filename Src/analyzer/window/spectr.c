...
//extern float rfft_mags[NSAMPLES/2]; //not extern, just static in fftwnd.c
float *rfft_mags;
...
unsigned long MaxiFreq, fmaxSIhz = CFG_GetParam(CFG_PARAM_SI5351_MAX_FREQ);
    ...
    if(AggregatedPoints>1){
        for(actFreq=upper+18000;actFreq>=lower-18000; ){// 3 ms per step
            ...
            if(graph==1){
                ...
                if(k>20){
                    k=0;
                    sprintf(f,"F: %d kHz     ",  (int)actFreq/1000);
                    FONT_Write(FONT_FRANBIG, TextColor, BackGrColor, 132, 100, f);
                }
            }
            actFreq-=17500ul;
            if(x<28) break;
   ...
   if(binx!=0){
    ...
    if(graph==1){
        ...
        else sprintf(f,"%.2f MHz ", (float)testFreq/1000);
        if(x<235) k=235;
        else k=45;
